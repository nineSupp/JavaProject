 ...
 * ID: 2420210037
 * PA 7.4: Supernova.java
 *
 * ==================================================================================
 * From the lyrics of the song "Supernova" by aespa, there is a catchy part with the
 * phrase "ah-oh, ayy". Count the number of occurrences of "ah-oh, ayy" that appear in
 * the lyrics.
 *
 * Given the following song lyrics code:
 *
 * String text = "I'm like some kind of supernova\n" + "Watch out\n" + "Look at me go,
 * 재미 좀 볼\n" + "빛의 core, so hot, hot\n" + "문이 열려 서로의 존재를 느껴\n" + "마치 Discord,
 * 날 닮은 너 (incoming!), 너 누구야? (Drop)\n" + "사건은 다가와, ah-oh, ayy\n" + "거세게 커져가,
 * ah-oh, ayy\n" + "That tick, that tick, tick bomb\n" + "That tick, that tick, tick
 * bomb\n" + "감히 건드리지 못할 걸 (누구도 말이야)\n" + "지금 내 안에선 (su-su-su-supernova)\n"
 * + "Nova, can't stop hyperstellar\n" + "원초 그걸 찾아\n" + "Bring the light of a dying
...
public class Supernova {
    public static void main(String[] args) {
        ...
        String toCheck = "ah-oh, ayy";

        for(int i = 0; i < text.length(); i++) {
            if (text.charAt(i) == 'a') {
                if (text.startsWith(toCheck, i)) {
                    countOccurrences++;
                }
            }
        }
        System.out.printf("There are %d occurrences of 'ah-oh, ayy'\n", countOccurrences);
